                    ...
                    <nav aria-label="breadcrumb" class=" rounded-3 p-3">
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item"><a href="{{ route('jobs') }}"><i class="fa fa-arrow-left"
                                        aria-hidden="true"></i> &nbsp;Back to Tuitions</a></li>
                        </ol>
                    </nav>
                </div>
            </div>
                    ...
                    <div class="card shadow border-0">
                        ...
                        <div class="descript_wrap white-bg">
                            ...
                            @if (!empty($job->responsibility))
                                <div class="single_wrap">
                                    <h4>Responsibility of Tutor</h4>
                                    {!! nl2br($job->responsibility) !!}
                                </div>
                            @endif
                            @if (!empty($job->qualifications))
                                <div class="single_wrap">
                                    <h4>Qualifications of Tutor</h4>
                                    {!! nl2br($job->qualifications) !!}
                                </div>
                            @endif
                            @if (!empty($job->benefits))
                                <div class="single_wrap">
                                    <h4>Benefits of the Student</h4>
                                    {!! nl2br($job->benefits) !!}
                                </div>
                            @endif
                            ...
                            <div class="pt-3 text-end">
                                ...
                                @if (Auth::check())
                                    @if ($applications->where('user_id', Auth::user()->id)->count() > 0)
                                        <a href="{{url('example2/'.$applications->where('user_id',Auth::user()->id)->first()->id)}}"
                                            class="btn btn-info">Payment</a>
                                    @else
                                        <a href="#" onclick="applyJob({{ $job->id }})"
                                            class="btn btn-info">Apply</a>
                                    @endif
                                @else
                                    <a href="javascript:void(0);" class="btn btn-info disabled">Login to Apply</a>
                                @endif
                    ...
                    <div class="card shadow border-0">
                        <div class="job_sumary">
                            <div class="summery_header pb-1 pt-4">
                                <h3>Tuition Summery</h3>
                                
                            </div>
                            <div class="job_content pt-3">
                                <ul>
                                    
                                    <li>Subject: <span> {{ $job->jobType->name }}</span></li>
                                    <li>Class: <span> {{ $job->category->name }}</span></li>
                                    <li>Posted on:
                                        <span>{{ \Carbon\Carbon::parse($job->created_at)->format('d M, Y') }}</span>
                                    </li>
                                    <li>Student No : <span>{{ $job->vacancy }}</span></li>
